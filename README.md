...
`decompose_unequal_representation()` can be used with either
candidate-level results or data that has already been aggregated to
party-by-district totals. When candidate-level results are supplied, the
function internally sums votes and seats by party within each district.
...
the data by election and apply `decompose_unequal_representation()` to
